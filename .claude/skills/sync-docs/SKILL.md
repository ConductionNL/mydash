...
Run the holistic structure review across all files in `{GITHUB_REPO}/docs/claude/` — looking at how docs relate to each other (overlap, differentiation, cross-references, proliferation). The 4 checks and depth guidance live in [references/doc-structure-review.md](references/doc-structure-review.md). Read it before starting Part C.
