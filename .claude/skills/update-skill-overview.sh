...
for dir in "$SCRIPT_DIR"/*/; do
  ...
  eval_count=0; has_trigger_tests=0; has_last_validated=0
  ...
  [ -f "$dir/learnings.md" ] && has_dated_learnings=$(grep -cE "^- (\[2|2[0-9]{3}-)" "$dir/learnings.md" 2>/dev/null || echo 0)
  has_capture=0
  ...
  if [ "$eval_count" -ge 3 ]; then
    ...
    [ "$enough_triggers" = "1" ] && [ "$has_last_validated" = "1" ] && l5_met=1
  fi
