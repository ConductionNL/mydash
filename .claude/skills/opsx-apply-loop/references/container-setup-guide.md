...
**Setup** (one-time):
```bash
chmod +x hydra/.claude/skills/opsx-apply-loop/assets/apply-loop-check.sh
```

**Monitoring loop** — run this command repeatedly (timeout: 120000ms per invocation):
```bash
hydra/.claude/skills/opsx-apply-loop/assets/apply-loop-check.sh "apply-loop-{APP}-{CHANGE_NAME}-{ITERATION}-t{TEST_ITERATION}" "$(pwd)/{APP}" "${LOG_DIR}/apply-loop-${RUN_TIME}-live.log"
```
