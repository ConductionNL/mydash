...
- **items** (array, required) — the hierarchical menu tree (max 3 levels deep)
- **style** (`dropdown` | `megamenu` | `tree`, default `dropdown`) — visual style
- **orientation** (`horizontal` | `vertical`, default `horizontal`) — bar layout for `dropdown`/`megamenu` (forced `vertical` for `tree`)
- **showIcons** (boolean, default `true`) — when false, all icon slots collapse to invisible spacers
- **expandedByDefault** (boolean, default `false`) — only meaningful for `tree` style
- **activeItemHighlight** (`background` | `underline` | `left-bar` | `none`, default `underline`) — visual treatment for the active item and its ancestors

Each item carries:

- **label** (string, required) — display text
- **url** (string, optional) — destination URL (internal route or external URL)
- **icon** (string, optional) — MDI registry name OR `/`-/`http`-prefixed URL (REQ-MENU-010)
