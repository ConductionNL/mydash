...
| Field | Type | Notes |
|-------|------|-------|
| `dashboardUuid` | string(36) | The dashboard the row aggregates. |
| `viewBucket` | DATE (UTC) | Calendar date in UTC; one row per (dashboard, day). |
| `viewCount` | int | Total view events on this date. |
| `uniqueViewerCount` | int | Distinct viewers on this date (cache-deduped). |

The composite `(dashboardUuid, viewBucket)` is unique — every additional view on the same day increments counters in place rather than inserting a new row.

Unique-viewer dedup uses a 32-byte random salt regenerated each UTC midnight by `SaltRotationJob`, persisted to `IConfig` under `mydash.analytics_dailysalt`, and **overwritten** (no history kept). The viewer hash is `sha256(userId || dailySalt)` and is stored only in the cache, keyed `mydash_anlt_<dashboardUuid>_<hash>` with TTL set to the seconds remaining until the next UTC midnight so cache and salt lifetimes line up.

Two settings gate tracking:

| Setting | Scope | Default | Effect when off |
|---------|-------|---------|-----------------|
| `mydash.analytics_enabled` | global | `true` | All view events are no-ops. |
| `mydash.analytics_optout` | per-user pref | `false` | This user's view events are no-ops. |
