...
**Constraint**: Exactly one of `userId` or `groupId` MUST be set (mutually exclusive XOR). The XOR is enforced at the service layer (`RoleService::validateTarget`); database-level CHECK constraints are not portable across the SQL dialects Nextcloud supports.

**Unique Constraint**: Per-target uniqueness — a user or group may have at most one assignment of each role type. Implementation: composite UNIQUE on `(user_id, role)` and `(group_id, role)`. Because each row populates exactly one of `user_id` / `group_id`, a single row only participates in one of the two unique indexes.
...
- **Resolution helper.** `RoleService` consumes `AdminTemplateService::getUserGroupIdsFor()` (the routing-resolver helper required by REQ-TMPL-013) when collecting group memberships, rather than calling `IGroupManager::getUserGroupIds` directly. The grep guard `AdminTemplateServiceGrepGuardTest` enforces that invariant.
- **PermissionService integration.** Viewer role is a hard short-circuit for every mutation method (`canEditDashboard`, `canAddWidget`, `canRemoveWidget`, `canStyleWidget`, `canCreateDashboard`). Admin role overrides the group-membership check inside `resolveAccessLevel` so a Dashboard Admin can edit `group_shared` dashboards without being a member of the target group. Editor role is granted full access on `group_shared` dashboards via the existing membership path inside `getEffectivePermissionLevel`. Personal-dashboard creation is granted to Editor or Admin regardless of the `allow_user_dashboards` admin flag.
- **Cascade entry points.** `UserDeletedListener` and `GroupDeletedListener` invoke `RoleService::deleteByUserId` / `deleteByGroupId` in best-effort try/catch blocks; a role-cascade failure is logged but never aborts the surrounding event chain (e.g. share-cleanup, ownership-transfer).
- **Database portability.** The XOR `(userId XOR groupId)` constraint is enforced at the service layer rather than as a SQL CHECK constraint because Nextcloud's supported dialects (MySQL ≥ 5.7, MariaDB, PostgreSQL, SQLite) do not all support partial / functional unique indexes uniformly.
