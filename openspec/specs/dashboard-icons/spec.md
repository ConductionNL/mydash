...
Own the icon vocabulary used by every MyDash dashboard surface (sidebar
switcher, admin list, dashboard create/edit picker). The capability
exposes a small curated frontend registry of named built-in icons plus
the lookup/render helpers that consumers use, so adding or removing an
icon happens in one place and every consumer stays in lock-step. The
backend stores `dashboards.icon` as an opaque string and never inspects
it — discrimination between registry name and uploaded URL lives in the
frontend (the URL branch is the foundation that the sibling capability
`custom-icon-upload-pattern` builds on).
