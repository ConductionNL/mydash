...
- **sourceType** (`'youtube' | 'vimeo' | 'peertube' | 'nc-file' | null`) — discriminator for the render branch and parser dispatch
- **videoUrl** (string) — canonical embed URL (server-parsed at save time) for hosted platforms; for `nc-file` this echoes the user-supplied path for transparency
- **fileId** (number | null) — Nextcloud file ID for `nc-file` source type
- **fileStreamingUrl** (string) — ACL-checked streaming URL produced by the file controller at render time (not persisted; populated by the parent before passing to the renderer)
- **autoplay** (bool, default `false`)
- **muted** (bool, default `true`) — coerced to `true` whenever `autoplay` is `true`
- **loop** (bool, default `false`)
- **controls** (bool, default `true`)
- **aspectRatio** (`'16:9' | '4:3' | '1:1' | '9:16'`, default `'16:9'`)
- **posterUrl** (string, default `''`) — applied to HTML5 `<video poster>` only
- **error** (string, optional) — set by the parent when the parse or ACL check fails; presence triggers the error placeholder
