...
- **internalCalendars** (`string[]`, default `[]`) — NC Calendar principal/key URIs to read from
- **externalIcsUrls** (`string[]`, default `[]`) — HTTPS ICS feed URLs to merge in
- **viewMode** (`'month' | 'week' | 'agenda'`, default `'agenda'`) — initial render mode
- **daysAhead** (`number`, default `14`) — look-ahead window for `agenda` mode
- **colorByCalendar** (`boolean`, default `true`) — whether to apply per-calendar colour accents

The events endpoint returns a flat array of normalised event objects with shape:
