...
Files-widget placements use the existing `oc_mydash_widget_placements.styleConfig` JSON column with the discriminated shape `{type: 'files', content: {...}}`. No schema migration is required.

The `content` object carries up to nine fields:

- **folderPath** (string) — absolute Nextcloud path like `/Documents/Marketing`
- **fileId** (number, nullable) — persistent folder id (PREFERRED over `folderPath` because it survives renames)
- **viewMode** (`list` | `grid` | `tree`, default `list`) — display mode
- **showThumbnails** (boolean, default `true`) — only honoured in `list` and `grid` modes
- **mimeTypeFilter** (string[], default `[]`) — `image/*`, `application/pdf`, etc.
- **allowUpload** (boolean, default `false`) — placement-level upload toggle
- **allowDelete** (boolean, default `false`) — placement-level delete toggle
- **sortBy** (`name` | `modified` | `size` | `type`, default `name`) — sort field
- **sortDescending** (boolean, default `false`) — sort direction

At least one of `folderPath` or `fileId` MUST be present so the placement always resolves to a folder at view time.
