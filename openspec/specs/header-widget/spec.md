...
- **backgroundImageUrl** (string|null, optional) — external image URL (`http(s)://` only)
- **backgroundImageFileId** (number|null, optional) — Nextcloud file ID; takes precedence over `backgroundImageUrl`
- **backgroundColor** (string, optional) — CSS color used as the solid background and the image-failure fallback (defaults to `var(--color-primary)`)
- **overlayMode** (`none` | `tint` | `gradient-bottom`, default `tint` when an image is present, otherwise `none`)
- **overlayColor** (string, optional) — overlay tint color (falls back to `backgroundColor`, then `#000000`)
- **overlayOpacity** (number, default `0.4`, clamped to `[0, 1]`)
- **textColor** (string, optional) — explicit override; otherwise auto-contrast picks `#ffffff` on dark backgrounds and `#000000` on light backgrounds
- **textAlign** (`left` | `center` | `right`, default `center`)
- **verticalAlign** (`top` | `middle` | `bottom`, default `middle`)
- **height** (`small` | `medium` | `large` | `xlarge`, default `medium`) — maps to `120 / 200 / 320 / 480` pixels
- **cta** (`{label: string, url: string, style: 'primary'|'secondary'|'ghost'}` | null, default `null`) — optional call-to-action button
