...
- **itemLimit** (`int` 1-50, default `10`) — server-side hard ceiling 200 regardless.
- **showThumbnails** (`bool`, default `true`).
- **showSummary** (`bool`, default `true`).
- **summaryMaxChars** (`int` 0-5000, default `200`).
- **dateFormat** (`'relative' | 'absolute'`, default `'relative'`).
- **metadataFilter** (`{fieldKey: string, value: string} | null`, default `null`).
