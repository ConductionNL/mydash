...
Fields: `schemaVersion` (integer, currently `1`), `exportedAt` (ISO 8601 UTC), `exportedBy` (Nextcloud user ID), `mydashVersion` (informational), `scope` (`"dashboard"` or `"site"`), `dashboardCount` (integer), `includedAssets` (array of strings).

### dashboards/<uuid>.json

Each dashboard is a JSON object with complete state — UUID, owner, name, description, type, basedOnTemplate, gridColumns, permissionLevel, targetGroups, isDefault, isActive, createdAt, updatedAt, parentUuid, slug, sortOrder, publicationStatus, widget placements (full grid config, style config, custom title/icon, sort order), and metadata field assignments.

### metadata-fields.json

Array of field definitions (`id`, `key`, `name`, `type`, `required`, `default`, `pattern`, `createdAt`) referenced by exported dashboards.
