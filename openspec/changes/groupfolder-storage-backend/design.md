...
A key structural fact from the reference app is that its `the source app_page_index` table stores only metadata — `unique_id`, `title`, `path`, `language`, `status`, `modified_at`, `parent_id`, `file_id` — and carries **no cached body/content**. The DB is never a content fallback; it is only a lookup index. Page bodies always require a live filesystem read. This is the architectural anchor for the decision below.
...
- `the source codebase-source/lib/Controller/ApiController.php:209-214` — listing endpoint catches `"the source app folder not found"` and returns `[]` rather than an error, but this is the list path only; all other endpoints propagate the exception as 404 or 500.
- `the source codebase-source/lib/Service/PageService.php:441-445` — `getthe source appFolder()` throws a plain `\Exception("the source app folder not found…")` with no fallback; caller in `getPage()` maps it to HTTP 404 (`ApiController:258-263`).
- `the source codebase-source/lib/Service/SetupService.php:79-89` — `setupSharedFolder()` returns `['success' => false, 'error' => 'groupfolders_app_not_enabled']` immediately when the app is absent; no alternative path attempted.
- `the source codebase-source/lib/Service/SetupService.php:562-569` — `isSetupComplete()` is a boolean predicate (`try { getSharedFolder(); return true; } catch { return false; }`); callers treat `false` as a blocking condition (see `DemoDataService:570-572` which runs setup first, not a different content source).
- `the source codebase-source/lib/Migration/Version001300Date20260420000000.php:25-76` — `the source app_page_index` schema: only `unique_id`, `title`, `path`, `language`, `status`, `modified_at`, `parent_id`, `file_id`. **No `body` or `content` column.** The DB is purely a metadata index; there is nothing to fall back to for content even if dual-read were desired.
...
- `the source codebase-source/lib/Migration/SetupDemoData.php:34-48` — setup runs before any content operations; if it fails, the repair step returns early with a warning, not by falling back to a different store.
- `the source codebase-source/lib/Service/SetupService.php:269-297` — `getSharedFolder()` throws `\Exception("Groupfolder '…' not accessible")` with no auto-create; auto-create is only in `setupSharedFolder()` → `createOrGetGroupfolderByName()`.
...
- `the source codebase-source/lib/Service/SetupService.php:625-703` — both migration helpers (`migrateResourcesFolders`, `migrateTemplatesFolders`) are purely additive; no removal of existing data after creating new structures.
