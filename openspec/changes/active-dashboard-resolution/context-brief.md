# Context Brief: Active-dashboard resolution chain

**App:** MyDash — Dashboard system
**Spec:** active-dashboard-resolution
...
**Rationale:** Resolution-chain plumbing  
...
### Configure dashboard access per user or group via modal
**demand: 56** (17 tender mentions) | Category: security

CAPABILITY: As a dashboard admin, I want to configure dashboard access per user or group via a modal, so that access control is managed efficiently without leaving the dashboard view. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard exists, WHEN the admin opens the access configuration modal, THEN a list of users and groups is displayed with their current access levels.
2. GIVEN the access modal is open, WHEN the admin grants or revokes access for a user or group and saves, THEN the change is applied immediately and reflected in the modal.
3. GIVEN access has been restricted, WHEN a user without access attempts to view the dashboard, THEN they are denied access and see an appropriate message.
