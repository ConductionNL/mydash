# Context Brief: Custom-icon URL pattern

**App:** MyDash — Dashboard system
**Spec:** custom-icon-upload-pattern
...
CAPABILITY: As a dashboard administrator, I want to upload custom CSS and brand icons, so that mydash matches my organisation's visual identity. ACCEPTANCE CRITERIA:
1. GIVEN I am a dashboard administrator, WHEN I upload a CSS file via the branding settings panel, THEN all dashboard pages reflect the custom styles without a page reload.
2. GIVEN I am a dashboard administrator, WHEN I upload an icon file (PNG or SVG), THEN the custom icon replaces the default icon across all views and browser tabs.
3. GIVEN an uploaded CSS file contains invalid syntax, WHEN I attempt to save the changes, THEN the system rejects the file and displays a validation error without altering the current styles.

### workspace-custom-icon
**demand: 3** | Category: general
Discovered from GitHub issue in Budibase/budibase

### Customizable backgrounds & icons
