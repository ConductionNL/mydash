---
capability: org-navigation
delta: false
status: draft
---

# Organization-wide Navigation Specification

## Purpose

This capability provides a robust, admin-curated, group-aware organization-wide navigation tree distinct from the personal dashboard list. The system persists trees as per-language JSON files, filters visibility based on Nextcloud group membership, renders a responsive runtime panel, and exposes an admin editor UI with language switching and position configuration.
...
Tree structure:
- Stored as ordered array of root nodes
- Max tree depth (root → children → grandchildren): 3 levels
- Max file size: 5 MB per language
- Stored at `IAppData('mydash')/org-navigation/{lang}.json` (one file per language: `nl.json`, `en.json`)

### Global Position Setting

- Stored in `mydash_admin_settings` with key `'org_navigation_position'`
- Type: string (enum: `'left'`, `'right'`, `'top'`, `'hidden'`)
- Default value: `'hidden'` (opt-in, no auto-render on first install)
- Applies globally to all users (per-user preferences are out of scope)

---

## Requirements

### REQ-ONAV-001: Org navigation tree storage
...
The tree is an ordered array of node objects. Tree depth (including root) MUST NOT exceed 3 levels.
...
### REQ-ONAV-002: Admin read API with group filtering
...
### REQ-ONAV-003: Admin write API with validation
...
### REQ-ONAV-004: Global position setting
...
### REQ-ONAV-005: Vue panel rendering
...
### REQ-ONAV-008: Empty tree and no-visible-nodes handling
...
### REQ-ONAV-009: Active item detection
...
### REQ-ONAV-010: Mobile responsive layout
...
### REQ-ONAV-011: URL sanitisation and validation
...
### REQ-ONAV-012: Internationalization
