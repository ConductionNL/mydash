# Context Brief: Widget context menu

**App:** MyDash — Dashboard system
**Spec:** widget-context-menu
...
### Remove an unused widget from the dashboard
**demand: 46** (14 tender mentions) | Category: analytics

CAPABILITY: As a dashboard user, I want to remove an unused widget from my dashboard, so that my workspace stays clean and relevant. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard with at least one widget, WHEN the user selects the option to remove a widget, THEN the widget is removed from the dashboard view.
2. GIVEN a widget has been removed, WHEN the user reloads the dashboard, THEN the removed widget does not reappear.
3. GIVEN a dashboard with multiple widgets, WHEN the user removes one widget, THEN all other widgets remain unaffected and visible.

### context-menu
**demand: 3** | Category: general
Discovered from GitHub issue in Budibase/budibase

### Configure settings for an individual widget
