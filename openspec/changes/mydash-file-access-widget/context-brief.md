# Context Brief: Dashboard document and file access

**App:** MyDash — Dashboard system
**Spec:** dashboard-file-access
...
**demand: 49** (14 tender mentions) | Category: document-management | **Coverage: mydash**

CAPABILITY: As a dashboard user, I want to open a dossier document directly from the dashboard, so that I can access relevant documents without navigating away. ACCEPTANCE CRITERIA:
1. GIVEN a dossier with associated documents exists, WHEN the dashboard user views the dossier on the dashboard, THEN a clickable document link or button is visible.
2. GIVEN a clickable document is displayed on the dashboard, WHEN the dashboard user clicks the document, THEN the document opens in the document viewer.
3. GIVEN the dashboard user has no access rights to the document, WHEN the dashboard user attempts to open it, THEN access is denied and an appropriate message is displayed.
