# Context Brief: Dashboard switcher sidebar

**App:** MyDash — Dashboard system
**Spec:** dashboard-switcher-sidebar
...
**Lives at:** Dashboards / Left sidebar
...
## Features (12 total, sorted by market demand)

### Rename an existing dashboard profile
**demand: 44** (14 tender mentions) | Category: analytics

CAPABILITY: As a dashboard user, I want to rename an existing dashboard profile, so that I can keep my profiles clearly identified as my needs evolve. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard profile exists, WHEN the user initiates a rename action on that profile, THEN an editable field pre-populated with the current name is presented.
2. GIVEN the rename field is active, WHEN the user submits a new non-empty name, THEN the profile is saved with the new name and displayed immediately in the profile list.
3. GIVEN the rename field is active, WHEN the user submits a blank or duplicate profile name, THEN an error message is shown and the original name is preserved.

### Delete an unused dashboard profile
**demand: 44** (14 tender mentions) | Category: analytics

CAPABILITY: As a dashboard user, I want to delete an unused dashboard profile, so that my workspace remains organised and clutter-free. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard profile exists and is not set as active, WHEN the user selects delete on that profile, THEN the profile is permanently removed from the list.
2. GIVEN a dashboard profile is currently active, WHEN the user attempts to delete it, THEN the system prevents deletion and displays an explanatory message.
3. GIVEN a dashboard profile has been deleted, WHEN the user views the profile list, THEN the deleted profile no longer appears.

### Switch to a different saved dashboard profile
**demand: 42** (14 tender mentions) | Category: analytics

CAPABILITY: As a dashboard user, I want to switch between saved dashboard profiles, so that I can instantly change my analytics view context. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard user has multiple saved profiles, WHEN they open the profile selector, THEN all saved profiles are listed and selectable.
2. GIVEN a dashboard user selects a different saved profile, WHEN the switch is confirmed, THEN the dashboard updates to display the layouts and widgets of the selected profile.
3. GIVEN a dashboard user switches profiles, WHEN the switch completes, THEN the previously active profile is preserved unchanged and can be returned to.

### WCAG AA compliance
**demand: 3** | Category: governance
...
### Switch between named dashboards via tabs or dropdown
