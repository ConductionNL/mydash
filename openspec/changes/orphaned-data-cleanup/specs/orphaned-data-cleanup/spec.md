---
capability: orphaned-data-cleanup
delta: false
status: draft
---
...
Administrators MUST be able to scan for and safely remove orphaned MyDash data — expired locks, share tokens for deleted dashboards, widget placements with missing dashboards, and more — via CLI commands, REST API, and daily background automation. The system MUST support dry-run (safe preview), per-category selectivity (which orphan types to purge), background auto-purge of the safest categories, and audit logging. Extensibility via registry pattern: adding a new cleanup category requires only creating one new class implementing `CleanupCategoryInterface`; no changes to orchestrator, CLI, controller, or background job are needed.

## Cleanup Categories

The system supports nine orphan cleanup categories, grouped into three safety tiers:

**Tier-A (Auto-safe, always shipped):**
- `expired_locks`: rows in `oc_mydash_dashboard_locks` where `updated_at` is older than 15 minutes
- `expired_share_tokens`: rows in `oc_mydash_dashboard_shares` whose `dashboard_id` does not exist in `oc_mydash_dashboards`

**Tier-B (Manual-safe, core shipped):**
...
**Tier-B (Manual-safe, optional features):**
- `orphaned_widget_assets`: files in `MyDash/Imports/*` and `MyDash/icons/*` not referenced by any dashboard or widget config (feature: optional, available when file backend is provisioned)
- `orphaned_metadata_values`: rows in `oc_mydash_metadata_values` where `fieldId` does not exist in `oc_mydash_metadata_fields` (feature: dashboard-metadata-fields, optional)
- `orphaned_feed_tokens`: rows in `oc_mydash_feed_tokens` where `userId` does not exist in `oc_users` (feature: dashboard-rss-feeds, optional)
- `dangling_dashboard_translations`: rows in `oc_mydash_dash_translations` where `dashboardUuid` does not exist in `oc_mydash_dashboards` (feature: dashboard-language-content, optional)

**Tier-C (Inspect-first, optional feature):**
- `orphaned_role_assignments`: rows in `oc_mydash_role_assignments` where `userId` (or `groupId`) does not exist in `oc_users` (or `oc_groups`) (feature: admin-roles, optional)

Categories for optional features report `isAvailable() === false` when the feature is not installed; the orchestrator skips them silently.
