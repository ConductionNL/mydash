# Context Brief: Enterprise security and access controls

**App:** MyDash — Dashboard system
**Spec:** enterprise-security-access
...
### Enterprise guest/role perms
**demand: 561** | Category: security | **Coverage: mydash**

CAPABILITY: As an IT admin, I want enterprise guest and role-based permissions, so that external collaborators access only authorised dashboards and data. ACCEPTANCE CRITERIA:
1. GIVEN a guest user invited to a workspace, WHEN they sign in, THEN they see only dashboards explicitly shared with their guest role.
2. GIVEN an admin assigning a custom role, WHEN they define permission scopes, THEN those scopes are enforced on every dashboard, widget, and data source the role accesses.
3. GIVEN a role's permissions are revoked, WHEN the affected user reloads the app, THEN previously visible dashboards and actions are no longer available.

### View as another role
**demand: 560** | Category: security | **Coverage: mydash**

CAPABILITY: As an IT admin, I want to view my dashboard as another role, so that I can verify role-based visibility and troubleshoot access issues. ACCEPTANCE CRITERIA:
1. GIVEN I am an IT admin on my dashboard, WHEN I select "View as" and pick a target role, THEN the dashboard re-renders showing only widgets and data visible to that role.
2. GIVEN I am viewing the dashboard as another role, WHEN the impersonation is active, THEN a persistent banner shows the assumed role and offers a one-click "Exit view" control.
3. GIVEN I am viewing as another role, WHEN I exit the view or my session ends, THEN the dashboard returns to my own role and the impersonation event is recorded in the audit log.
