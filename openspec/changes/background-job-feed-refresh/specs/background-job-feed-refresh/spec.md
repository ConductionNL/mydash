---
capability: background-job-feed-refresh
delta: true
status: draft
---

# Background Job Feed Refresh — Delta from change `background-job-feed-refresh`

## Context

The MyDash news widget allows users to configure feeds from multiple RSS 2.0 and Atom 1.0 URLs. A background job periodically discovers active feed URLs from widget placements, fetches and parses them, caches the results in the database, and provides an admin endpoint for on-demand refresh.

The job MUST:
- Run on a configurable interval (default 60 minutes)
- Scale to hundreds of feeds via batch processing with cursor-based resumption
- Isolate failures (one feed timeout does not block others)
- Use HTTP conditional-get headers to minimize bandwidth
- Maintain a distributed lock to prevent concurrent instances
- Enforce a host allow-list if configured
- Prepare for future orphan cleanup via a mapper interface

Backend exports:

| Export | Type | Purpose |
|---|---|---|
| `FeedRefreshService::refreshAll($feedUrl?)` | Method | Refresh all feeds or single URL; returns stats |
| `FeedCacheMapper::findOrphanedBefore($cutoff)` | Method | Query feeds unused since cutoff (for sibling cleanup job) |
| `FeedParserInterface` | Interface | Pluggable feed parser; can be swapped without touching job |
