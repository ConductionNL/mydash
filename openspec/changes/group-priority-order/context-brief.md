# Context Brief: Group priority order — admin setting

**App:** MyDash — Dashboard system
**Spec:** group-priority-order
...
### Audience targeting by user groups
**demand: 10** (1 tender mentions) | Category: other

CAPABILITY: As a communications officer, I want to target dashboard content to specific user groups, so that each audience sees only relevant information. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard widget exists, WHEN the communications officer assigns it to one or more user groups, THEN only members of those groups see the widget on their dashboard.
2. GIVEN a user belongs to multiple groups, WHEN they view their dashboard, THEN they see the combined content targeted to all their groups with no duplicates.
3. GIVEN a user is removed from a group, WHEN they next load their dashboard, THEN content exclusively targeted to that group is no longer visible to them.

### Restrict widget connection to user groups
**demand: 9** (1 tender mentions) | Category: other

CAPABILITY: As a dashboard admin, I want to restrict widget connections to specific user groups, so that data access is controlled by group membership. ACCEPTANCE CRITERIA:
1. GIVEN a widget with no group restrictions, WHEN a dashboard admin assigns it to specific user groups, THEN only members of those groups can connect and view the widget.
2. GIVEN a widget restricted to certain groups, WHEN a user outside those groups accesses the dashboard, THEN the widget is not visible or connectable to that user.
3. GIVEN a group-restricted widget, WHEN a dashboard admin removes a group from the restriction list, THEN members of that group immediately lose access to the widget connection.
