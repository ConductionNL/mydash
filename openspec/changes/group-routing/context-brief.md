# Context Brief: Group routing — pick primary group for a user
...
**demand: 8** | Category: security

CAPABILITY: As a communications officer, I want to target dashboard content to users by role, so that each role sees relevant widgets and landing pages. ACCEPTANCE CRITERIA:
1. GIVEN a widget is configured with a role restriction, WHEN an employee with a matching role logs in, THEN that widget is visible on their dashboard.
2. GIVEN a widget is configured with a role restriction, WHEN an employee without a matching role logs in, THEN that widget is not displayed on their dashboard.
3. GIVEN role-based landing pages are configured, WHEN an employee logs in, THEN they are directed to the landing page assigned to their role.

### Audience targeting
**demand: 7** | Category: other

CAPABILITY: As a communications officer, I want to target dashboard content to specific audiences, so that each group sees only relevant information. ACCEPTANCE CRITERIA:
1. GIVEN a content block exists, WHEN the communications officer assigns one or more audience segments to it, THEN only users belonging to those segments see the block on their dashboard.
2. GIVEN multiple audience segments are defined, WHEN a user belongs to more than one segment, THEN they see all content blocks targeted to any of their segments.
3. GIVEN a content block has no audience targeting applied, WHEN any user views the dashboard, THEN the block is visible to all users.

### Group-based visibility
**demand: 2** | Category: other

### Segments (multi-audience)
**demand: 2** | Category: other
