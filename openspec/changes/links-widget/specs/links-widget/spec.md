...
The links widget is a built-in MyDash widget type that renders a multi-column grid of link cards organised into named sections. Each section carries a heading and an arbitrary number of links, each link carries a label, URL, optional icon, and optional description, and the renderer offers three layout modes (`card`, `inline`, `icon-only`) optimised for different density preferences. All configuration lives in the placement `widgetContent` JSON — there is no backend data endpoint and no migration. URL sanitisation runs at save time to defend against `javascript:`, `data:`, and `file://` XSS vectors.

## Data Model

Links widget placements use the existing `oc_mydash_widget_placements.widgetContent` JSON column with the discriminated shape `{type: 'links', content: {...}}`. No schema migration is required for the placement table.

The `content` object carries the following fields:

- **sections** (`array`, default `[]`) — array of section objects, each with `title: string` and `links: array`
- **columns** (`number`, default `3`) — CSS Grid column count, range 1–6
- **linkLayout** (`'card' | 'inline' | 'icon-only'`, default `'card'`) — link rendering mode
- **iconSize** (`'small' | 'medium' | 'large'`, default `'medium'`) — icon dimension mode
- **openInNewTab** (`boolean`, default `true`) — open links in new tab
- **showSectionTitles** (`boolean`, default `true`) — show section headings
- **showLinkDescriptions** (`boolean`, default `true`) — show link descriptions (card mode only)

Each link object carries:

```json
{
  "label": "string (required, non-empty)",
  "url": "string (required, non-empty, sanitised)",
