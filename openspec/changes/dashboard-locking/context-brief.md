...
**Implemented** — entity (`lib/Db/DashboardLock.php`), mapper (`lib/Db/DashboardLockMapper.php`), service (`lib/Service/DashboardLockService.php`), controller (`lib/Controller/DashboardLockApiController.php`), migration (`lib/Migration/Version001021Date20260502130000.php`), exceptions (`LockConflictException`, `LockNotFoundException`, `LockForbiddenException`), routes (`appinfo/routes.php`), frontend composable (`src/composables/useDashboardLock.js`), API methods (`src/services/api.js`), and i18n (`l10n/{en,nl}.{js,json}`). Cascade-delete is wired into `DashboardService::deleteDashboard()` via the optional `DashboardLockMapper` constructor argument.
