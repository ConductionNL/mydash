---
capability: dashboard-reactions
delta: false
status: implemented
---
...
Stored in `oc_mydash_dashboard_reactions` table. One row per (dashboardUuid, userId, emoji) tuple.

- `id` (integer, PK) — surrogate key
- `dashboardUuid` (string, FK) — UUID of the dashboard, references `oc_mydash_dashboards.uuid`
- `userId` (string) — Nextcloud user ID of the reactor
- `emoji` (string) — Unicode emoji (e.g., `👍`, `❤️`, `🎉`)
- `reactedAt` (datetime) — ISO timestamp when reaction was created

**Uniqueness:** (dashboardUuid, userId, emoji) is unique. One user can react with multiple distinct emojis to the same dashboard, but cannot duplicate the same emoji.

**Indices:**
- UNIQUE (dashboardUuid, userId, emoji)
- INDEX dashboardUuid
- INDEX emoji

### Dashboard Extension

`oc_mydash_dashboards` gains one nullable SMALLINT column:
- `reactionsEnabled` — NULL = follow global setting; 1 = reactions enabled; 0 = reactions disabled

### Admin Settings

- `mydash.reactions_enabled_default` (boolean, default: true) — global toggle
- `mydash.reactions_allowed_emojis` (JSON array of strings, default: `["👍","❤️","🎉","😂","🤔","😢"]`) — emoji whitelist

---

## Requirements

### REQ-RXN-001: Add Reaction
...
### REQ-RXN-003: Get Reactions Summary
...
### REQ-RXN-004: List Reactors by Emoji
...
- THEN the system MUST return HTTP 200 with the first 100 results plus a `nextCursor` value
- AND a follow-up request with `?cursor=100` MUST return rows 100..149 with `nextCursor: null`
...
- THEN the system MUST return HTTP 200 with `{items: [], nextCursor: null, total: 0}`
...
### REQ-RXN-005: Global Reactions Toggle
...
### REQ-RXN-006: Per-Dashboard Reactions Toggle
...
### REQ-RXN-007: Allowed Emoji Whitelist
...
### REQ-RXN-008: Permission Enforcement
...
### REQ-RXN-009: Cascade Delete on Dashboard Deletion
...
- **Localization**: All error messages (`"Emoji not allowed"`, `"Reactions are disabled"`, etc.) MUST support English and Dutch per i18n requirements.
- **Cascade safety**: Deleting a dashboard MUST reliably cascade-delete all reactions via `ReactionsListener` subscribing to `DashboardDeletedEvent`.
