...
`openspec validate` exits clean. All 13 event types produce correctly structured `oc_activity` rows; debounce suppresses reactions and global fan-out within the 15-minute window; cross-capability emission contract documented in `Extension.php` docblock.

## Tests (company-wide ADR-008)

PHPUnit per Tasks 12.1–12.5 (ExtensionTest.php). All 13 event-type × 2 subject-variant combinations asserted (26 total); debounce suppression verified with mock APCu.

## Documentation (company-wide ADR-009)

Changelog entry covering the Activity feed integration, the 13 event types, audience-targeting rules, and the opt-out path in NC Settings → Activity.

## i18n (company-wide ADR-007)

`nl_NL` + `en_US` for all 13 first-person and third-person subject templates, plus message template for `dashboard_commented`.
