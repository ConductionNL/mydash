...
This change introduces no new OpenRegister schemas and no new database tables. NC Activity persists events in the existing `oc_activity` table managed by NC core. No seed data is required for this capability.

## Spec Changes Implied

- **REQ-ACT-002 (event catalogue)**: The 13 listed types in `spec.md` are correct — `dashboard_created`, `dashboard_updated`, `dashboard_deleted`, `dashboard_published`, `dashboard_unpublished`, `dashboard_scheduled`, `dashboard_shared`, `dashboard_public_share_created`, `dashboard_commented`, `dashboard_reacted`, `dashboard_restored`, `dashboard_lock_overridden`, `dashboard_role_changed`. Confirm no `dashboard_viewed` constant appears anywhere. The 5 source-derived types are ports and the 8 additions are MyDash-native.
- **REQ-ACT-003 (audience targeting)**: The four-scope table from D3 is normative. Group membership is resolved at emit-time, not read-time.
- **REQ-ACT-007 / REQ-ACT-008 (debouncing)**: The 15-minute (900 s) APCu TTL is the canonical window value. Normative in the requirement body, not just in scenarios.
- **NOTE**: Source-derived types and MyDash additions coexist by design. The spec is not incomplete because it defines more types than the source; the additions are intentional extensions for capabilities the source does not have.
