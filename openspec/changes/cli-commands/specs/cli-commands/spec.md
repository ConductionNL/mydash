...
When `--json` is passed, the command's stdout MUST be a single JSON object with:
...
**Fields:**
- `success` (boolean): `true` if no errors; `false` if any unrecovered error occurred.
- `exitCode` (integer): One of 0, 1, 2, 3, 4, 5.
- `data` (object | array | null): Command-specific output. `null` on error.
...
1. **`appinfo/info.xml`** — add a `<commands>` block listing each command's fully qualified class name
2. **Command class** — each extends `CommandBase` (which extends `OC\Core\Command\Base`):
...
- WHEN the dashboard has child dashboards
...
- THEN `data/nextcloud.log` MUST contain: `[mydash] cli dashboard:list --user=alice exitCode=0 durationMs=<ms> byUser=admin`
...
- THEN `data/nextcloud.log` MUST include: `[mydash] cli dashboard:delete a1b2c3d4 exitCode=3 durationMs=<ms> byUser=<uid>`
...
- GIVEN a pre-existing MyDash command uses a different naming convention
...
- GIVEN a developer opens a PR adding a new occ command
- WHEN code review checks compliance with this spec
- THEN the PR MUST be rejected unless the command is renamed to comply with the namespace pattern
