# Context Brief: Default-dashboard flag per group

**App:** MyDash — Dashboard system
**Spec:** default-dashboard-flag
...
**Lives at:** Beheer / Tab: Groups & Routing

**Rationale:** Per-group routing config  
...
**demand: 85** (14 tender mentions) | Category: analytics

CAPABILITY: As a dashboard admin, I want a default dashboard configuration applied to all new users, so that every user starts with a consistent, pre-configured view. ACCEPTANCE CRITERIA:
1. GIVEN a default dashboard configuration has been saved in mydash, WHEN a new user account is created, THEN the user's dashboard displays the pre-configured widgets and layout without manual setup.
2. GIVEN a default dashboard configuration exists, WHEN an admin updates the default config, THEN newly created users receive the updated layout while existing users retain their current dashboards.
3. GIVEN a new user logs in for the first time, WHEN the dashboard loads, THEN all default widgets are visible, correctly positioned, and functional with no errors.

### Reset a specific user's dashboard to group default
**demand: 50** (15 tender mentions) | Category: analytics

CAPABILITY: As a group admin, I want to reset a specific user's dashboard to the group default, so that the user's view is restored to the standard configuration. ACCEPTANCE CRITERIA:
1. GIVEN a user has a customised dashboard, WHEN the group admin selects that user and triggers "Reset to group default", THEN the user's dashboard layout matches the current group default.
2. GIVEN a group default dashboard exists, WHEN the reset is applied, THEN the targeted user's personal overrides are removed and the group default takes effect on their next login.
3. GIVEN the group admin resets a user's dashboard, WHEN the action completes, THEN all other users' dashboards remain unchanged.

### Reset dashboard layout to department default
**demand: 43** (14 tender mentions) | Category: analytics

CAPABILITY: As a dashboard user, I want to reset my dashboard layout to the department default, so that I can restore a consistent shared starting point. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard user has a customised layout, WHEN they trigger "Reset to Department Default", THEN the layout reverts to the current department default configuration.
2. GIVEN the reset is triggered, WHEN the action completes, THEN a confirmation prompt is shown before any changes are applied.
3. GIVEN the layout has been reset, WHEN the user views the dashboard, THEN all widget positions and sizes match the department default exactly.

### Create default dashboard layout per organisational group
**demand: 42** (14 tender mentions) | Category: analytics

CAPABILITY: As an IT admin, I want to create a default dashboard layout per organisational group, so that each group sees a relevant, consistent view on login. ACCEPTANCE CRITERIA:
1. GIVEN an organisational group exists, WHEN the IT admin assigns a default dashboard layout to that group, THEN all members of the group see that layout upon next login.
2. GIVEN a default layout is set for a group, WHEN a new user is added to that group, THEN their dashboard initialises with the group's default layout.
3. GIVEN multiple organisational groups with different default layouts, WHEN a user belongs to one group, THEN only their group's default layout is applied, not layouts from other groups.

### group-based defaults
**demand: 1** | Category: other

### Set a profile as the default on login
