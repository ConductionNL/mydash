# Specifications — video-widget

This document formalizes the requirements from `context-brief.md` as a specification reference for the video-widget capability. All requirements use the `REQ-VID-NNN` format with `GIVEN`/`WHEN`/`THEN` scenarios.

## REQ-VID-001: Register Video Widget with Nextcloud Dashboard API

**Functional**: The system MUST register a video widget with Nextcloud's Dashboard API so it appears in widget discovery and can be added to dashboards.
...
## REQ-VID-002: Store Video Widget Configuration per Placement

**Functional**: The widget placement's `widgetContent` JSON MUST store source type, URL/file ID, and display options.
...
## REQ-VID-003: Support Multiple Video Source Types

**Functional**: The widget MUST handle YouTube, Vimeo, PeerTube, and Nextcloud Files as distinct source types with appropriate rendering logic.
...
## REQ-VID-004: Enforce Admin-Controlled Domain Allowlist

**Functional**: The system MUST prevent embedding videos from arbitrary domains unless explicitly allowed by the administrator.
...
## REQ-VID-005: Parse and Validate Video URLs Server-Side

**Functional**: The backend MUST extract video IDs from user-provided URLs and validate format before storage.
...
## REQ-VID-006: Check File Access Control for Nextcloud Files

**Functional**: When `sourceType` is `nc-file`, the system MUST verify the viewing user can read the file before allowing playback.
...
## REQ-VID-007: Use iframe with CSP-Safe Sandbox Attributes

**Functional**: Hosted video platforms (YouTube, Vimeo, PeerTube) MUST be embedded via iframe with strict sandbox restrictions.
...
## REQ-VID-008: Respect Autoplay, Muting, and Loop Settings

**Functional**: The widget MUST apply user-selected playback options while respecting browser autoplay policies.
...
## REQ-VID-009: Enforce Aspect Ratio via CSS

**Functional**: The widget MUST apply the configured aspect ratio using modern CSS and fallback techniques.
...
## REQ-VID-010: Support No-Cookie YouTube Embedding

**Functional**: The system MUST support an optional admin setting to enable YouTube no-cookie embedding to reduce tracking.
...
## REQ-VID-011: Display Appropriate Empty and Error States

**Functional**: The widget MUST show user-friendly messages for missing config, access issues, and invalid domains.
