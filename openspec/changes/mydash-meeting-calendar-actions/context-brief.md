# Context Brief: Meeting and calendar actions from dashboard

**App:** MyDash — Dashboard system
**Spec:** meeting-calendar-actions
...
**Lives at:** Catalog / Widget type (Calendar)
...
**demand: 42** (14 tender mentions) | Category: core | **Coverage: nextcloud → `nc:calendar`**

CAPABILITY: As a dashboard user, I want to annotate agenda items on the dashboard via the Nextcloud Calendar app, so that I can add context or notes to scheduled events at a glance. ACCEPTANCE CRITERIA:
1. GIVEN an agenda item is visible on the dashboard, WHEN the user adds an annotation to it, THEN the annotation is saved and displayed alongside the agenda item.
2. GIVEN an annotated agenda item on the dashboard, WHEN the user edits the annotation, THEN the updated text is reflected immediately without a page reload.
3. GIVEN an annotated agenda item on the dashboard, WHEN the user removes the annotation, THEN the annotation is deleted and the agenda item returns to its unannotated state.
