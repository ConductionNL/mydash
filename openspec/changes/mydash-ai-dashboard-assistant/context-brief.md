# Context Brief: Embedded AI assistant and smart summaries
...
**demand: 42** (14 tender mentions) | Category: ai | **Coverage: mydash**

CAPABILITY: As a case worker, I want to view a summary of my open cases on the dashboard, so that I can quickly assess my current workload. ACCEPTANCE CRITERIA:
1. GIVEN a case worker has open cases, WHEN they view the dashboard, THEN a summary widget displays the total count of their open cases.
2. GIVEN a case worker has open cases, WHEN they view the dashboard summary, THEN each case shows at minimum its identifier, status, and last updated date.
3. GIVEN a case worker has no open cases, WHEN they view the dashboard, THEN the summary widget displays a message indicating no open cases exist.

### View consultation response summary on dashboard
**demand: 42** (14 tender mentions) | Category: ai | **Coverage: mydash**

CAPABILITY: As a dashboard user, I want to view a summarized overview of consultation responses on my dashboard, so that I can quickly assess feedback without navigating away. ACCEPTANCE CRITERIA:
1. GIVEN a consultation has received responses, WHEN the user views the dashboard, THEN a summary of consultation responses is displayed as a distinct widget or panel.
2. GIVEN the summary is displayed, WHEN the user reviews it, THEN key metrics (e.g. total responses, response breakdown) are visible without additional clicks.
3. GIVEN consultation response data changes, WHEN the dashboard is loaded or refreshed, THEN the summary reflects the current response data.
