# Context Brief: Fork-current-layout to personal dashboard

**App:** MyDash — Dashboard system
**Spec:** fork-current-as-personal
...
**Lives at:** Dashboards / Top-bar action

**Rationale:** Inline action  
...
**demand: 526** | Category: other

CAPABILITY: As a dashboard user, I want to customize my dashboard layout, so that I can arrange widgets to match my workflow. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard with multiple widgets, WHEN I drag a widget to a new position, THEN the widget moves and the new layout persists across sessions.
2. GIVEN I am viewing my dashboard, WHEN I resize a widget, THEN the widget adopts the new dimensions and surrounding widgets reflow accordingly.
3. GIVEN I have customized my layout, WHEN I reload the dashboard or sign in from another device, THEN my saved layout is restored for my user account.

### Per-user layout persistence
**demand: 20** (1 tender mentions) | Category: other

CAPABILITY: As a dashboard user, I want my layout saved per account, so that my arrangement is restored on every login. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard user has customised their layout, WHEN they log out and log back in, THEN the same layout is displayed without manual reconfiguration.
2. GIVEN two dashboard users with different layouts, WHEN each logs in, THEN each sees only their own saved arrangement.
3. GIVEN a dashboard user modifies their layout, WHEN the change is saved, THEN subsequent sessions reflect the updated layout, not a previous version.

### per-user widget config
**demand: 20** (1 tender mentions) | Category: other

CAPABILITY: As a dashboard user, I want to configure each widget independently per account, so that my layout persists across sessions. ACCEPTANCE CRITERIA:
1. GIVEN a dashboard user has customized a widget's settings, WHEN they log out and log back in, THEN their widget configuration is restored exactly as saved.
2. GIVEN two dashboard users on the same system, WHEN each configures the same widget differently, THEN each user sees only their own configuration.
3. GIVEN a dashboard user modifies a widget setting, WHEN the change is saved, THEN no other user's widget configuration is affected.
