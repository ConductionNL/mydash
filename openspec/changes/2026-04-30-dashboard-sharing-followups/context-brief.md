...
### Configure a daily email digest of dashboard notifications
**demand: 42** (14 tender mentions) | Category: analytics | **Coverage: nextcloud → `nc:mail`**

CAPABILITY: As a dashboard user, I want to configure a daily email digest of notifications via the Nextcloud Mail app, so that I stay informed without checking the dashboard constantly. ACCEPTANCE CRITERIA:
1. GIVEN a user has dashboard notifications enabled, WHEN they enable the daily digest option, THEN a summary email is sent to their configured address once per day.
2. GIVEN the daily digest is enabled, WHEN no notifications occurred that day, THEN no email is sent for that period.
3. GIVEN a daily digest email is delivered, WHEN the user opens it, THEN it lists each notification with its title, source, and timestamp from the past 24 hours.
