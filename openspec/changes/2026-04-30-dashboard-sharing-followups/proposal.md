...
- Listen to Nextcloud's `OCP\User\Events\UserDeletedEvent`. When user `X` is deleted:
  ...
  2. **For every dashboard owned by X**, evaluate the *admin pool*:
     - The admin pool consists of every share row on that dashboard whose `permission_level = 'full'`, expanded as: each `user`-type share contributes one user; each `group`-type share contributes the current member list (resolved via `IGroupManager`) at deletion time.
...
- This logic runs synchronously in the event listener inside a single DB transaction per dashboard.
- Group deletion is **not** handled by this change — a follow-up periodic cleanup job is out of scope here.
