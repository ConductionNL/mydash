...
Market demand (from context-brief.md): 42 tender mentions for daily email digest, 8 for in-app notifications, 3 for integrated notifications, 2 for notify users. All four demand signals are satisfied by a single `INotifier` implementation, which delivers via the Nextcloud bell, activity stream, and daily email digest without any custom transport.

## Reuse Analysis

This change intentionally avoids building any custom notification transport, webhook handler, or audit mechanism. The following platform services are leveraged directly:

| Service | Provider | Usage |
|---|---|---|
| `OCP\Notification\IManager` | Nextcloud core | Publishing share + ownership-transfer notifications |
| `OCP\Notification\INotifier` | Nextcloud core | Rendering notifications in bell, activity stream, and email digest |
| `OCP\User\Events\UserDeletedEvent` | Nextcloud core | Triggering the cascade + ownership-transfer logic |
| `OCP\IGroupManager` | Nextcloud core | Expanding group shares to member lists at fan-out time |
