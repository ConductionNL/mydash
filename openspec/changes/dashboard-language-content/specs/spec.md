...
| `dashboardUuid` | VARCHAR(36) | References `oc_mydash_dashboards.uuid` | Identifies the parent dashboard |
| `languageCode` | VARCHAR(16) | ISO 639-1 base code (`nl`, `en`, `de`, `fr`) | Normalised 2-character language code |
| `name` | VARCHAR(255) | Not null | Localised dashboard name |
| `description` | TEXT | Nullable | Localised dashboard description |
| `widgetTreeJson` | TEXT | Not null | Localised widget tree as JSON |
| `isPrimary` | SMALLINT (0/1) | Default 0, service-enforced exactly one per dashboard_uuid = 1 | Primary variant flag |
| `createdAt` | DATETIME | Not null, default current timestamp | Creation time |
| `updatedAt` | DATETIME | Not null, default current timestamp, updated on write | Last modification time |

### Indexes

| Name | Columns | Type | Purpose |
|---|---|---|---|
| PRIMARY | `id` | Primary key | Entity identity |
| `mydash_trans_dash_idx` | `(dashboard_uuid)` | Index | Variant lookups by dashboard |
| `mydash_trans_unique_idx` | `(dashboard_uuid, language_code)` | Unique | Enforces one row per (dashboard, language) pair |
| `mydash_trans_primary_idx` | `(dashboard_uuid, is_primary)` | Index | Accelerates primary-variant lookup |
...
- THEN the `oc_mydash_dash_translations` table MUST exist with all columns listed in the Data Model section
...
- AND the user's Nextcloud locale is set to `'nl'`
- WHEN `GET /api/dashboards/{uuid}` is called (no `?lang` param)
- THEN the system MUST return the `'nl'` variant
...
- GIVEN a dashboard has variants for `'en'`, `'nl'`, `'de'`, `'fr'`
- WHEN `GET /api/dashboards/{uuid}` is called
- THEN the response MUST include `availableLanguages: ['de', 'en', 'fr', 'nl']` (sorted alphabetically)
...
- AND all 3 translation rows MUST be cascade-deleted via `DashboardTranslationService::deleteAllForDashboard()`
...
- **Localization**: All new error messages MUST be translatable to Dutch and English per the i18n requirement (ADR-007).
- **Backwards compatibility**: Dashboards created before the translation feature MUST continue to work without manual intervention; the legacy materialiser keeps reads working until a subsequent translation API call.
