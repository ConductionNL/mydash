...
The image widget is a built-in MyDash widget type that lets dashboard authors place a single image — logo, screenshot, branding, or decorative imagery — onto a dashboard cell with proper `object-fit` control, broken-image fallback, optional click-through, and a three-branch UX supporting direct URL entry, file upload to managed storage, and Nextcloud Files picker. It replaces the prior workarounds where users jammed `<img>` tags into the markdown widget or pointed an iframe widget at an image URL.

The capability is one widget type, one renderer, one multi-branch form, one registry entry, and thin client/server wrappers around the resource-uploads and Nextcloud preview endpoints owned by the `resource-uploads` and `core` capabilities respectively.
...
- **sourceType** (string, default `'url'`) — one of `'url'`, `'upload'`, `'files'`; chooses which input branch is active in the edit form (REQ-IMP-001)
- **fileId** (number | null, default `null`) — Nextcloud file ID set when `sourceType` is `'files'` (REQ-IMP-003)
- **filePath** (string, default `''`) — human-readable Nextcloud file path (max 2048 chars) set when `sourceType` is `'files'` (REQ-IMP-003); shown read-only under the picker button so the user knows what they selected

Resource lifecycle (orphan GC, quota accounting) is owned by the `resource-uploads` capability and a future GC change. For `sourceType: 'files'` the source bytes live in Nextcloud Files and are owned by Nextcloud's standard file lifecycle — the widget keeps only the `fileId` and `filePath` reference, never copies the bytes.
...
- WHEN the widget renders
- THEN the system MUST call `IURLGenerator::linkToRoute('core.preview.getPreview', ['fileId' => 12345, ...])` (or use the equivalent client-side preview URL form)
...
- THEN the URL generation service MUST return `null` (server) or the preview request MUST return 404 (client)
...
- **Access Control**: Preview URL generation MUST respect Nextcloud's file ACLs; viewers without permission MUST receive `null` URL or a 404 from the preview service, not a 403 error. The widget gracefully falls back (REQ-IMP-005).
...
- **Localization**: All new form labels ("Pick from Files", "Image source", etc.) and error messages MUST be available in English and Dutch (nl/en) per the i18n requirement.
