...
- [x] 1.2 Migration is reversible (DROP TABLE + cascading foreign keys cleaned up)
- [x] 1.3 Test migration locally on SQLite, MySQL, PostgreSQL; verify schema applied cleanly

## 2. Domain model

- [x] 2.1 Create `lib/Db/DashboardView.php` entity with fields:
  - `$id` (int)
...
- [x] 2.2 Add `DashboardView::jsonSerialize()` for API responses

## 3. Mapper layer

- [x] 3.1 Create `lib/Db/DashboardViewMapper.php` extending `QBMapper` with methods:
...
- [x] 3.2 Add PHPUnit test for mapper covering: upsert with increment, find by dashboard, find in range, delete older than, empty results

## 4. Cache layer for unique-viewer dedup

- [x] 4.1 Create `lib/Service/UniqueViewerDedup.php` service with:
...
- [x] 5.1 Create `lib/Service/AnalyticsService.php` with methods:
...
- [x] 5.2 Test via PHPUnit: all query methods with fixtures, CSV generation correctness, period parsing, empty result handling

## 6. Controller + endpoints

- [x] 6.1 Add `viewEvent()` method to existing `lib/Controller/DashboardController.php`:
...
- [x] 6.3 Register routes in `appinfo/routes.php`:
...
- [x] 7.2 Register job in `appinfo/app.php` via `\OCP\BackgroundJob\IJobList::add(PurgeViewsJob::class)`
...
- [x] 9.1 Add action to `src/stores/dashboards.js`:
...
- [x] 9.2 Add action to `src/stores/admin.js` (or create if missing):
...
- [x] 9.3 Store state for analytics UI: `topDashboards`, `dashboardDetail`, `instanceSummary`, `loading`, `error`

## 10. Frontend instrumentation

- [x] 10.1 Update `src/views/DashboardView.vue`:
...
- [x] 11.1 Create `src/views/AdminAnalytics.vue` (new) displaying:
  - "Analytics" header
...
- [x] 11.2 Create `src/components/AnalyticsChart.vue` (new) displaying 30-day sparkline:
...
- [x] 11.3 Integrate AdminAnalytics into main admin navigation:
...
- [x] 13.1 `DashboardViewMapperTest`:
...
- [x] 13.4 `DashboardControllerTest::viewEvent`:
...
- [x] 13.5 `AnalyticsControllerTest`:
...
- [x] 14.1 User calls `POST /api/dashboards/{uuid}/view-event`, receives 204
- [x] 14.2 Admin calls `GET /api/admin/analytics/dashboards/top?period=7d`, receives array
- [x] 14.3 Admin calls `GET /api/admin/analytics/dashboards/{uuid}?period=30d`, receives daily breakdown
- [x] 14.4 Admin calls `GET /api/admin/analytics/summary?period=30d`, receives totals + top 5
- [x] 14.5 Admin calls `GET /api/admin/analytics/export?period=30d`, downloads CSV
- [x] 14.6 Non-admin receives 403 on admin endpoints
- [x] 14.7 Opted-out user's POST view-event returns 204 with no counter change
- [x] 14.8 Vue component calls `POST /api/dashboards/{uuid}/view-event` on mount (verify via network log)
- [x] 14.9 Multi-tab debouncing: open same dashboard in two tabs, verify only ONE request sent

## 15. Quality gates

- [x] 15.1 `composer check:strict` (PHPCS, PHPMD, Psalm, PHPStan) passes
- [x] 15.2 ESLint + Stylelint clean on all Vue/JS files
- [x] 15.3 SPDX headers on all new PHP files (inside docblock per SPDX-in-docblock convention)
- [x] 15.4 `i18n` translation keys for all new UI strings (nl + en):
  - "View Analytics"
  - "Top Dashboards"
  - "Total Views"
  - "Unique Viewers"
...
- [x] 15.5 Update generated OpenAPI spec / Postman collection with 5 new endpoints
- [x] 15.6 Run all `hydra-gates` locally before opening PR

## 16. Documentation

- [x] 16.1 Update `README.md` or `docs/ANALYTICS.md` with:
