---
capability: footer-customization
status: draft
---
...
Each setting MAY contain language-tagged variants as a nested map: `{en: "...", nl: "..."}`. Render logic selects matching viewer locale; falls back to first key if no match.

### Dashboard Footer Fields
...
- **AND** the result MUST contain the allowed content
...
- **GIVEN** a dashboard owned by "alice" with `view_only` permission for user "bob"
...
Footer content MUST support language-tagged variants. The footer renderer MUST select the matching variant using a three-step fallback chain:
...
#### Scenario: Dashboard API returns footer fields
