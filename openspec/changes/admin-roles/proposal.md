...
- Effective role resolution: Nextcloud admins always have MyDash "admin" role. Non-admins resolve their effective role via direct user assignment (takes precedence as-is) or highest-privilege group assignment. Default (no assignment) = no role, falls back to existing `permissions` capability.
...
- `lib/Listener/UserDeletedListener.php` (existing or new) — call `RoleService::deleteByUserId()` on user deletion
- `lib/Listener/GroupDeletedListener.php` (existing or new) — call `RoleService::deleteByGroupId()` on group deletion
