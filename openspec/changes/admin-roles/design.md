...
Admin-roles is therefore a net-new MyDash design with no source counterpart. It introduces three roles scoped entirely within MyDash (Dashboard Admin, Dashboard Editor, Dashboard Viewer), persistent DB-backed assignments, and a highest-privilege-wins resolution algorithm for group assignments, with direct user assignment taking precedence when present. The existing `permissions` capability is extended to consult the role system before falling back to its default GroupFolder-derived behavior.
...
- Resolve effective role per user using a deterministic algorithm: NC admin first; then direct user assignment as-is if present; then highest-privilege-wins over group assignments.
...
- Cascade cleanly on Nextcloud user or group deletion via listener hooks registered through the `dashboard-cascade-events` capability infrastructure.
...
### D4: Effective-role resolution algorithm
...
3. If no direct user assignment exists, collect all group assignments for groups the user is a member of. Map roles to a numeric rank: "admin"=2, "editor"=1, "viewer"=0. Effective role = the assignment with the highest rank.
4. If no assignments exist → effective role is null; falls back to `permissions` capability behavior.

**Rationale**: The direct-assignment-wins-over-groups rule gives administrators a predictable explicit override mechanism — if you assign a user "viewer" directly, that is intentional and should not be silently overridden by their group memberships. Highest-privilege-wins for group assignments is intuitive (belonging to a higher-privileged group should not be negated by another lower-privileged group), consistent with GroupFolder ACL union semantics, and easy to implement (MAX over role ranks). NC admin is the unconditional root override, consistent with Nextcloud's own privilege model.
...
**Decision**: The setup-wizard sibling capability MAY include an optional step — "Bootstrap roles from existing NC groups?" — that creates one role assignment per role, binding each to a selected NC group. Recommended for greenfield installs but not required.
...
Example `oc_mydash_role_assignments` rows (Dutch organisational context):

| id | userId        | groupId         | role   | assignedBy | assignedAt          |
|----|---------------|-----------------|--------|------------|---------------------|
| 1  | `NULL`        | `beheerders`    | admin  | admin      | 2026-01-15 09:00:00 |
| 2  | `NULL`        | `redacteuren`   | editor | admin      | 2026-01-15 09:01:00 |
| 3  | `NULL`        | `medewerkers`   | viewer | admin      | 2026-01-15 09:02:00 |
| 4  | `jan.de.vries`| `NULL`          | admin  | admin      | 2026-02-03 11:30:00 |
| 5  | `noor.bakker` | `NULL`          | viewer | admin      | 2026-03-10 14:15:00 |

Notes on seed data:
- Row 1–3: group assignments for the three standard organisational groups.
- Row 4: individual user `jan.de.vries` explicitly granted admin (direct user assignment).
- Row 5: individual user `noor.bakker` pinned to viewer even though their group `redacteuren` grants editor — direct user assignment is used as-is and group assignments are skipped.

## Spec Changes Implied

- **REQ-ROLE-001**: NOTE confirms Dashboard Admin is a net-new MyDash capability with no source counterpart. NC admin status is an implicit grant — no assignment row is created.
- **REQ-ROLE-005**: Direct user assignment wins over group assignments (not simply "highest"); group assignments use highest-wins among groups; NC admin is unconditional override.
- **REQ-ROLE-009**: Scenario 1 (direct "viewer" beats group "admin") is consistent with REQ-ROLE-005 — direct user assignment is used as-is when present; groups consulted only when absent.
- **REQ-ROLE-010 / REQ-ROLE-011**: Cascade listeners extend `dashboard-cascade-events` capability infrastructure.

## Open Follow-ups

- Whether assigning a role to the literal `'default'` group sentinel should grant the role to every authenticated user — likely yes for symmetry, needs a REQ.
- Whether MyDash should expose a preview API ("what role would user X have if they joined group Y?") — out of scope, candidate for `admin-roles-v2`.
- Whether per-dashboard role overrides are ever needed (deliberately excluded as a Non-Goal above).
